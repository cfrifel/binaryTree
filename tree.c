 ...
 * data in nodes of the tree is a single text character
 * that ranges from 'a' to 'z'.  But initially, the tree is
 * empty.
 *
 * Then it gets commands from the user:
 *
 *   - (q) Quit
 *   - (e) Insert an alphabet letter
 *   - (d) Delete an alphabet letter
...
Node * maxTree(Node * nodeptr);
Node * getParent(char c, Node ** ptr2link);
...
void deleteNode(Node ** ptr2link) 
{
	...
	if (nodeptrptr == NULL) /* Couldn't find it */
	{
		return;
	}	
	...
	{
		*nodeptrptr = NULL; /* Delete the node from the tree  by NULLing the link from the parent node */
		destroyNode(nodeptr); /* Deallocate space back to memory */
	}
	else if (nodeptr->left != NULL && nodeptr->right == NULL) /* only left child */
	{
		*nodeptrptr = nodeptr->left;
		destroyNode(nodeptr);
	}
	else if (nodeptr->left == NULL && nodeptr->right != NULL) /* only right child */
	{
		*nodeptrptr = nodeptr->right;
		destroyNode(nodeptr);
	}
	else /* Has both a right and left child */
	{
		// find the predecessor to the node being deleted
		Node * predptr; 
		predptr = maxTree(nodeptr->left);
		
		// find the pointer to the parent of the predecessor node
		Node * parentptr;
		parentptr = getParent(predptr->data, ptr2link);
		
		if (parentptr->data == nodeptr->data)
		{
			predptr->right = nodeptr->right;
			*nodeptrptr = parentptr->left;
			destroyNode(nodeptr);
		}
		else
		{
			*nodeptrptr = parentptr->right;
			parentptr->right = predptr->left;
	
			predptr->left = nodeptr->left;
			predptr->right = nodeptr->right;
...
/*
 * Returns the pointer to a parent of a target node
 */
Node * getParent(char c, Node ** ptr2link)
{
	Node * parentptr;
	parentptr = *ptr2link;
	if (*ptr2link == NULL || parentptr->data == c)
	{
		return NULL;
	}

	if (parentptr->left != NULL && parentptr->left->data == c)
	{
		return parentptr;
	}
	else if (parentptr->right != NULL && parentptr->right->data == c)
	{
		return parentptr;
	}
	...
	if (parentptr->data > c)
	{
		getParent(c, &(parentptr->left));
	}
	else
	{
		getParent(c, &(parentptr->right));
	}
}
...
Node ** searchNode(char c, Node ** ptr2link)
{
	...
	if (*ptr2link == NULL) /* tree is empty */
	{
		return NULL;
	}
	else
